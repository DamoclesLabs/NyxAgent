...
export async function generateShouldRespond({
    ...
    while (true) {
        try {
            ...
            elizaLogger.debug("Raw response from model:", response);
            const parsedResponse = parseShouldRespondFromText(response.trim());
            if (parsedResponse) {
                elizaLogger.debug("Successfully parsed response:", parsedResponse);
                return parsedResponse;
            } else {
                elizaLogger.debug("Failed to parse response. Response was:", response);
                elizaLogger.debug("generateShouldRespond no valid response found");
            }
        } catch (error) {
