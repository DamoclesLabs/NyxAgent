...
export const parseShouldRespondFromText = (
    ...
    elizaLogger.debug("Attempting to parse response text:", text);

    const firstLine = text.split("\n")[0].trim();
    elizaLogger.debug("First line of response:", firstLine);

    const cleanedText = firstLine.replace("[", "").toUpperCase().replace("]", "");
    elizaLogger.debug("Cleaned text for matching:", cleanedText);

    const match = cleanedText.match(/^(RESPOND|IGNORE|STOP)$/i);
    elizaLogger.debug("Regex match result:", match);

    if (match) {
        const result = match[0].toUpperCase() as "RESPOND" | "IGNORE" | "STOP";
        elizaLogger.debug("Found exact match:", result);
        return result;
    }

    // Fallback to includes check
    elizaLogger.debug("No exact match, checking includes");
    if (text.includes("RESPOND")) {
        elizaLogger.debug("Found 'RESPOND' in text");
        return "RESPOND";
    }
    if (text.includes("IGNORE")) {
        elizaLogger.debug("Found 'IGNORE' in text");
        return "IGNORE";
    }
    if (text.includes("STOP")) {
        elizaLogger.debug("Found 'STOP' in text");
        return "STOP";
    }

    elizaLogger.debug("No valid response found in text");
    return null;
};
